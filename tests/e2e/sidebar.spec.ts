...
test("toggles the mobile node toolbox overlay", async ({ page, isMobile }) => {
  ...
  await expect(page.getByRole("complementary", { name: "Node toolbox" })).toHaveClass(/translate-x-full/);
  ...
  await expect(page.getByRole("complementary", { name: "Node toolbox" })).toHaveClass(/translate-x-0/);
