...
test("surfaces existing-turret deployment status without lifecycle actions", async ({ page, isMobile }) => {
  ...
  await expect(page.getByLabel("Deployment status details")).toBeVisible();
  await expect(page.getByLabel("Deployment status details")).toContainText(/Deployment blocked|Target:/i);
