...
Canonical Link: [turret.move](https://github.com/evefrontier/world-contracts/blob/main/contracts/world/sources/assemblies/turret.move)

This is the actual contract that controls the turret within the CCP world. It is the source of truth for how turrets work, and is what the game client interacts with when players build and use turrets in-game. The code is written in Move, a safe systems programming language developed by Meta (formerly Facebook) for smart contract development.
...
Canonical Link: [turret.move extension](https://github.com/evefrontier/world-contracts/blob/main/contracts/extension_examples/sources/turret.move)

This is an _example_ of an extension contract for custom turret targeting behaviour. It demonstrates how builders can create their own logic for prioritizing targets based on the information provided by the game client. The extension contract uses the typed witness pattern to integrate with the main turret contract, allowing it to receive target candidate information and return a customized priority list of targets for the turret to engage. This is the code that Frontier Flow would generate and deploy for builders.
