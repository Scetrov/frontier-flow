...
Remote publish compilation requirement: as established in [ADR-009](./ADR/ADR-009-deploy-grade-dependency-resolution.md), the `Published.toml` injection into a local shim is **insufficient for deploy-grade compilation** against live `world` packages. Deploy-grade bytecode must be compiled against real dependency-resolution inputs: the actual upstream `world` package files (`Move.toml`, `Move.lock`, source tree) from the pinned repository revision, derived `rootGit` metadata, and generated `resolvedDependencies`. The generated extension package must keep `builder_extensions = "0x0"` for its own address, but it must not redeclare `world = "0x0"` in the root package manifest when compiling the remote publish artifact. (The earlier `Published.toml` approach remains in the codebase as a transitional implementation pending the deploy-grade path.)
